...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if audios is not None:
            # 先给 hidden_states 做一次 +0，这样 new_hidden_states 不再是 leaf
            new_hidden_states = hidden_states + 0
            # 假设 audio_pos 里存储了 (batch_idx, start, end) 这类信息
            for idx, (i, a, b) in enumerate(audio_pos):
                # 对 new_hidden_states 做「局部写」
                new_hidden_states[i, a : b+1] = audios[idx]
            # 最后再统一替换回去
            hidden_states = new_hidden_states

        # if audios is not None:
        #     for idx, (i, a, b) in enumerate(audio_pos):
        #         # 原先： hidden_states[i][a : b+1] = audios[idx]
